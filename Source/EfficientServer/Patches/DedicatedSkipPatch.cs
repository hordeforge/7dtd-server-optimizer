        ...
        static void TryPrefix(string typeName, string methodName, string prefixName)
        {
            try
            {
                ...
                if (t == null)
                {
                    ...
                    EsLog.Emit(LogLevel.Warn, $"skip-patch {typeName}.{methodName}: type not found (skip disabled)");
                    return;
                }
                MethodInfo m = t.GetMethod(methodName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
                if (m == null)
                {
                    EsLog.Emit(LogLevel.Warn, $"skip-patch {typeName}.{methodName}: method not found (skip disabled)");
                    return;
                }
                ...
                EsLog.Emit(LogLevel.Info, $"skip-patch {typeName}.{methodName}");
            }
            catch (Exception ex)
            {
                EsLog.Emit(LogLevel.Warn, $"skip-patch {typeName}.{methodName} failed [{ex.GetType().Name}]: {ex.Message}");
            }
        }
