...
- [ ] Validate AI LOD behavior for alert, combat, sleeper, quest, and distant entities. (2026-08-09 static audit vs stock RE: `UpdateTasksLodPatch` never strides/skips attacking (GetAttackTarget), investigating (HasInvestigatePosition), alerted (GetAlertTicks>0), or active-sleeper entities - the combat/alert/sleeper set is structurally covered; `AiLodPatch`'s `aiActiveScale` only gates EAI cadence (locomotion always runs per RE) and the missing stock top-N quota is documented as accepted in FEATURES.md with a bounded residual. Live 2026-08-09: animator-emergency + path-admission harness PASSED on V3.1.0 (animoff entered CullCompletely on 123 rigs, animon restored all 123 with root motion intact - the combat-fidelity exit path is mechanically validated; path knobs reloaded live without horde collapse; see RESULTS 'Live animator-emergency + path-admission validation'). Remaining: the 64p + 200+z blood-moon stress A/B for a frame-win claim.)
